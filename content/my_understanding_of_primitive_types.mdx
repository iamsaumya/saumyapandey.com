...
How the heck are we calling length on `s`? The answer to this question is `Boxing` which JS does for itself. JS converts `s` into an object of `String` class and we call its methods.
...
All outputs are logical except for the last, `typeof null` is an `object`? Yes! This is supposed to be a bug in JavaScript and now they can fix it because it will break a lot of code. Now, it is very logical why `null` is not a primitive type.
...
**`NaN`**
Many of us think of it as an acronym for Not a Number, which is not correct. In Javascript,`NaN` is a special value that means an invalid number.
...
I hope these examples made sense and the last example is a possible way to create NaN.
...
Here is a custom method that will result in an incorrect value.
...
Weird! All the above operation seems to be weird. It kinda feels like they cheating. Triple equals lie to us, greater than and less than are behaving weird.
...
This is it from my side folks, I come back to discuss with on **coercion** in Javascript. I would love to hear from you, discuss with me on Twitter or ping me on LinkedIn. See you!
