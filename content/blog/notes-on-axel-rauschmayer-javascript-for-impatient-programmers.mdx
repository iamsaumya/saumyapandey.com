...
These are my notes from [Javascript for impatient programmers](https://exploringjs.com/impatient-js/). I highly recommend you to read through the book for better understanding of the concepts. However, if you are already familiar with JS this could be a great way resource to revise essentials things. 
...
### const
Variables declared using const are blocked scoped and immutable. They have to be declared and unitialised at the same point.
```js
{
    const i = 0;
...
```
When are saying that variable declared via const are immuatable we mean the binding between the variable name and its value. The value itself could be mutable.
...
The variables of the global scope are called __global variables__ and are accessable everywhere.

There are two types of global variables.

1. Global declarative variables.
	Normal variables declared using `let`, `const` or `class` declarations.
2. Global object variables.
	Variables declared using `var` or `function` declarations.

__globalThis__ is used to access global object variables. The reason why it got its name is because it is equal to `this` in global scope.
...
Declaration scope is the area where a variable or entity can be seen. Variables which are blocked scope have their declaration scope between the `{` and `}` they enclosed. 
...
Wherease, the variables declared with `var` are function scoped and they do not have TDZ. In case of variables declared with `var` will return `undefined` when accessed before their intializations.

Same goes with `function`, you could write function declaration at the bottow and make a function call at the top of the file. 

However, there is a caveat. `function` declared with `let` or `const` keyword have TDZ and cannot be accessed before their declarations. 
```js
...
- They are implementation of static scoping and helps to provide context data to callbacks.
...
Similary, `string` and `number` have infinitely many values.
...
Objects values are always __passed and compared by reference__.
...
Use `Object(value)` to convert value from a primitive to object type. 
...
When we perform an operation, the operator needs the operands in specific type. If the operands are not in the desired types, they are tried to convert into the desired type. Most of the time people don't bother to understand the significance of operand type and raise questiosn about unpredictibility of JS. 
...
### Loose equality ( == )
- It coerces its operands to primitives type if the other operand is primitive type.
- If both the operand are objects, they only equal if the objects are same.
```js 
undefined == null //true
```
### Strict Equality ( === )
- It doesn't coerces its operands.
```js
...
If the first operand is either `null` and `undefined` and return the second operand as a fallback.
```js
a ?? b
a !== undefined && a!== null ? a : b
```
### Comma Operator
Evaluate both expression and return the second one.
