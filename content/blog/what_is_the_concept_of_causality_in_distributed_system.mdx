...
![Distributed System](/static/images/what_is_the_concept_of_causality_in_distributed_system/DistributedSystem.png)
...
Knowing the time of occurrence of the events will help us in the above scenarios. Not to mention that there are million other such examples where the time of occurrence is essential.

In a centralized system, we have a single point of truth for all the incoming request and occurrences of the events that happen on the system. As a result, it is very intuitive to order events in centralized systems.

![Centralized System](/static/images/what_is_the_concept_of_causality_in_distributed_system/CentralizedSystem.png)

In the image above, we can assume that the three events have happened on 8:00, 8:30 and 8:55 repectively. 
...
However, we need to know the possible orderings of the events. There are two different types of ordering possible.
...
In distributed systems, it is easier to attain partial ordering between the series of events as they happen on different nodes concurrently. While we don’t always need to obtain total order, it is hard to attain when required.

![Ordering of Events](/static/images/what_is_the_concept_of_causality_in_distributed_system/ordering.png)
...
We know that if Node 1 handled your tweet 1, it will send a message to node two to inform about it. Node 2 needs some logic to infer that tweet 1 occurred before tweet 2 that just happened on node 2.
...
Let’s take a break. In the next blog, we will discuss how **the concept of causality is a potential casualty**. We will learn how **Lamport clocks** help us to determine causality between different events.
